...
pan_net_if_lo() {
	...
	if [ -z "$N_NET_IF_LOOPBACK" ] || [ $N_NET_IF_LOOPBACK -le 0 ]; then
		return 0
	fi

	echo -en "\nNetwork > Interfaces > Loopback ($N_NET_IF_LOOPBACK) with zone and VR assigned "
...
EOF
	...
	xpath="$XPATH/config/devices/entry[@name='$LHOST']/network/interface/loopback/units"
	xpath_vsys="$XPATH/config/devices/entry[@name='$LHOST']/vsys/entry[@name='$VSYS']/import/network/interface"
	xpath_zone="$XPATH/config/devices/entry[@name='$LHOST']/vsys/entry[@name='$VSYS']/zone/entry[@name='$IF_LOOPBACK_ZONE']/network/layer3"
	xpath_vr="$XPATH/config/devices/entry[@name='$LHOST']/network/virtual-router/entry[@name='$IF_LOOPBACK_VR']/interface"
	...
	for ((i = $IF_LOOPBACK_IP_OCTET_i; i < 256; i++)); do
		for ((j = $IF_LOOPBACK_IP_OCTET_j; j < 256; j++)); do
			if [ $interfaces -gt $N_NET_IF_LOOPBACK ]; then
				break 1
			fi

			interface_name=`printf "loopback.%d" $interfaces`
			interface_ip=`printf "$IF_LOOPBACK_IP" $i $j`
...
if [ -n "$N_NET_IF_LOOPBACK" ] && [ $N_NET_IF_LOOPBACK -gt 0 ]; then
