...
pan_net_if_tun() {
	...
	if [ -z "$N_NET_IF_TUNNEL" ] || [ $N_NET_IF_TUNNEL -le 0 ]; then
		return 0
	fi

	echo -en "\nNetwork > Interfaces > Tunnel ($N_NET_IF_TUNNEL) with zone and VR assigned "
...
EOF
	...
	if [ "$TARGET" = "PANORAMA" ]; then
		if [ -n "$PANORAMA_TEMPLATE" ]; then
			...
			template="$PANORAMA_TEMPLATE_STACK (Stack, which may not support interfaces)"
		else
	...
	xpath="$XPATH/config/devices/entry[@name='$LHOST']/network/interface/tunnel/units"
	xpath_vsys="$XPATH/config/devices/entry[@name='$LHOST']/vsys/entry[@name='$VSYS']/import/network/interface"
	xpath_zone="$XPATH/config/devices/entry[@name='$LHOST']/vsys/entry[@name='$VSYS']/zone/entry[@name='$IF_TUNNEL_ZONE']/network/layer3"
	xpath_vr="$XPATH/config/devices/entry[@name='$LHOST']/network/virtual-router/entry[@name='$IF_TUNNEL_VR']/interface"
	...
	for ((i = $IF_TUNNEL_IP_OCTET_i; i < 256; i++)); do
		for ((j = $IF_TUNNEL_IP_OCTET_j; j < 256; j++)); do
			if [ $interfaces -gt $N_NET_IF_TUNNEL ]; then
				break 1
			fi

			interface_name=`printf "tunnel.%d" $interfaces`

			if [ -z "$IF_TUNNEL_IP" ]; then
				element="<entry name='$interface_name'/>"
			else
				interface_ip=`printf "$IF_TUNNEL_IP" $i $j`
...
if [ -n "$N_NET_IF_TUNNEL" ] && [ $N_NET_IF_TUNNEL -gt 0 ]; then
